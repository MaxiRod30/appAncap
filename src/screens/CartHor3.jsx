...
import { usePostNotificationTercerTurnoMutation, useGetNotificationTercerTurnoQuery } from "../services/apiInfoServices";
...
const CartHor3 = ({ navigation }) => {
  const horario = "tercerTurno"
  ...
  const { data: dataNotif, error, isLoading } = useGetNotificationTercerTurnoQuery(localId );
  ...
  const [triggerAlarm, result] = usePostNotificationTercerTurnoMutation()
  ...
  useEffect(() => {
    if (!isLoading && dataNotif?.terceroTurno!== undefined) {
      const {msg, time } = dataNotif.terceroTurno
  ...
  useEffect(() => {
    ...
    if (alarmString !== null && alarmString !== dataNotif?.terceroTurno?.time) {
  ...
  useEffect(() => {
    if (result.isSuccess & alarmString !== null & dataNotif?.terceroTurno?.horario) {
      dispatch(
      setAlarmSelected({
        alarm: result.data.terceroTurno.msg,
        user: result.data.terceroTurno.localId,
        horario: result.data.terceroTurno.horario,
      })
      )
    }
  }, [result])
